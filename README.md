...
- 🎇 **Element Spotlights** — the first time you blast a *featured* element, a **cinematic full-screen reveal** plays: the orb **flies to center and morphs** into the element, a flash opens onto a real-world, element-themed world (Gold's vault, Carbon's diamonds, Neon's glowing tubes…), and **themed particles** swirl in — gold sparks, diamond glints, neon wisps, forge embers, rising oxygen bubbles — each landing with its own **signature sound** (gold's metallic ching, neon's electric whoosh, carbon's crystal shimmer, iron's forge clang…), alongside what it is and its real-world connection. (**16 elements shipped** — H, He, Li, C, O, Ne, Mg, Al, Si, Cl, Ca, Fe, Cu, Ag, Hg, Au — and extends to all 118. Images lazy-load, so the set adds no startup cost.)
